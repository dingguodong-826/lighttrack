...
from Evolution.src.Search.search import EvolutionSearcher
from lib.core.supernet_function_tracking import get_cand_with_prob, get_cand_head, get_oup_pos, get_cand_head_wo_ID

class EvolutionSearcher_DP(EvolutionSearcher):

    def __init__(self, args):
        super(EvolutionSearcher_DP, self).__init__(args)  # Inherit EvolutionSearcher
        self.args = args
        ...
        self.restrict_sample = args.restrict_sample
        #
    ...
    def get_one_cand(self):
        if self.restrict_sample:
            ...
            if np.random.random_sample() < 0.2:
                '''restrict not using ID'''
                cand_h_dict = {'cls': get_cand_head_wo_ID(), 'reg': get_cand_head_wo_ID()}
            else:
                cand_h_dict = {'cls': get_cand_head(), 'reg': get_cand_head()}
            # backbone output position
            cand_OP = get_oup_pos(self.sta_num)
            return (get_random_cand, cand_h_dict, cand_OP)
        else:
            # backbone
            get_random_cand = get_cand_with_prob(self.CHOICE_NUM, sta_num=self.sta_num)
            # add head and tail
...
def main():
    ...
    searcher = EvolutionSearcher_DP(args)
