...
def get_cand_flops_ablation(cand, mac_model_name='LightTrackM_FLOPs'):

    cand_b, cand_h_dict, cand_op = cand['back'], cand['head'], cand['ops']
    return compute_macs_DP(cand_b, cand_h_dict, cand_op, mac_model_name=mac_model_name)


def compute_macs(cand_b, cand_h_dict, mac_model_name='LightTrackM_FLOPs'):
    # model
...
'''2020.10.17'''
def compute_macs_DP(cand_b, cand_h_dict, backbone_index, mac_model_name='LightTrackM_FLOPs'):
    # model
    model_dict = MODEL_DICT[mac_model_name]
    mac_model, zf, search = model_dict['mac_model'], model_dict['zf'], model_dict['search']
    # extra operations
