...
class OceanDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # print('max value of template is %.4f'%np.max(template))
        # print('max value of search is %.4f'%(np.max(search)))
        '''Normalization'''
    ...
    def _create_dynamic_logisticloss_label(self, label_size, c_shift, rPos=2, rNeg=0):
        ...
        label = np.where(dist_to_center <= rPos,
                         ...
                         np.where(dist_to_center < rNeg,
                                  ...
                                  np.zeros_like(y)))
        return label
    '''2020.08.27 advanced version by Bin Yan'''
    # def _create_dynamic_logisticloss_label(self, label_size, c_shift, rPos=2, rNeg=0):
    #     if isinstance(label_size, int):
    #         sz = label_size
    #     else:
    #         sz = label_size[0]
    #     sz_ori = sz * self.stride
    #     # get center coord
    #     sz_x_ori = sz_ori / 2 - c_shift[0]
    #     sz_y_ori = sz_ori / 2 - c_shift[1]
    #     # get high-resolution score map
    #     x, y = np.meshgrid(np.arange(0, sz_ori) - sz_x_ori,
    #                        np.arange(0, sz_ori) - sz_y_ori)
    #
    #     dist_to_center = np.abs(x) + np.abs(y)  # Block metric
    #     score = np.where(dist_to_center <= rPos * self.stride,
    #                      np.ones_like(y),
    #                      np.where(dist_to_center < rNeg,
    #                               0.5 * np.ones_like(y),
    #                               np.zeros_like(y))).astype(np.float)
    #     # map to expected size
    #     score = cv2.resize(score, (sz,sz), interpolation=cv2.INTER_AREA)
    #     # get label
